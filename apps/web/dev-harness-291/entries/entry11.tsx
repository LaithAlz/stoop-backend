...
async function main() {
  ...
  render();
  await sleep(300);
  log("=== Skip reachability on Home while the #252 flag is up ===");
  log("pending, no flag:", state());
  byText("Edit")!.click(); await sleep(60);
  const ta = document.querySelector("textarea") as HTMLTextAreaElement;
  const setter = Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, "value")!.set!;
  setter.call(ta, "EDITED"); ta.dispatchEvent(new Event("input", { bubbles: true }));
  await sleep(50);
  byText("Send edited version")!.click();
  await sleep(400);
  log("flag up:", JSON.stringify(Array.from(getUnverifiedSendSnapshot().keys())));
  log("editor still open, controls:", state(), "<- no Skip rendered in the editing state");
  byText("Cancel")!.click(); await sleep(120);
  log("after Cancel, controls:", state());
  const skip = byText("Skip")!;
  log("Skip disabled while flagged:", skip.disabled, "(must be false)");
  log("card carries the explanation:", (document.querySelector("article")?.textContent ?? "").includes("Checking whether your last reply went out"));
  skip.click();
  await sleep(400);
  log("skip actually reached the server:", SERVER.rejects === 1);
  log("card after skip:", JSON.stringify((document.body.textContent ?? "").includes("No reply sent")));
  log("");
  log("=== does the undo ticket ever mount with undoDisabled=true (approve still busy)? ===");
  SERVER.items = [routine("p2", "BODY Q")];
  qc = createQueryClient(); root.unmount(); root = createRoot(document.getElementById("app")!); render();
  await sleep(400);
  const seen: string[] = [];
  const stop = setInterval(() => {
    const u = document.querySelector("button[aria-describedby]") as HTMLButtonElement | null;
    const v = u ? String(u.getAttribute("aria-busy")) : "none";
    if (seen[seen.length - 1] !== v) seen.push(v);
  }, 0);
  byText("Approve & send")!.click();
  await sleep(1200);
  clearInterval(stop);
  log("aria-busy sequence on the undo button from approve onward:", JSON.stringify(seen));
  log("(a 'true' before 'false' means the ticket DID mount undo-disabled, i.e. the first true->false");
  log(" transition is the APPROVE settling, not an undo settling)");
  log("DONE");
  flush();
}
void main();
