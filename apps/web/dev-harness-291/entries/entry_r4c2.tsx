...
globalThis.fetch = (async (input: any, init: any = {}) => {
  ...
  if (url.includes("/v1/cases/")) {
    if ((SERVER as any).caseDown) return json(500, { error: { code: "server_error", message: "x", request_id: "r" } });
...
async function main() {
  ...
  log("=== R4-C2: thread route, stale in-flight read + case endpoint then down ===");
  log("controls:", ctrls());
  ...
  (SERVER as any).caseDown = true;
  log("(case endpoint now returns 500 - the same outage class that makes a send ambiguous)");
