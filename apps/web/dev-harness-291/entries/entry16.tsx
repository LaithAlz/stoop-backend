...
async function main() {
  ...
  render();
  await sleep(300);
  log("=== 'Sent.' visibility when the row has ALREADY dropped out of items (the #291 pin case) ===");
  const t0 = Date.now();
  const events: string[] = [];
  let last = false;
  ...
  await sleep(300);
  log("mid-window: card pinned?", document.querySelectorAll("article").length, "| undo ticket:", document.querySelectorAll("button[aria-describedby]").length);
  await sleep(9000);
  obs.disconnect();
  log("'Sent.' mutation events:", JSON.stringify(events));
  log("cards left:", document.querySelectorAll("article").length);
  log("body tail:", JSON.stringify((document.body.textContent ?? "").replace(/\s+/g," ").slice(-140)));
  log("DONE");
  flush();
}
void main();
