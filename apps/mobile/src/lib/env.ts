 ...
 *
 * Adversarial safety review, 2026-08-04, item 2 (FIX 2, HIGH): "refuses to
 * boot" above is only true because src/app/_layout.tsx touches `env.apiUrl`
 * once at module load, before anything renders. Before that fix, nothing
 * accessed this getter until the first real API call, deep inside
 * src/api/client.ts's fetch try/catch, where the thrown Error was mapped to
 * the same generic `network_error` every dropped connection gets and never
 * reached a log or the landlord. If you ever remove that _layout.tsx touch,
 * this module's whole "fails loud at boot" claim goes back to being false.
 */
 ...
 *
 * Throws (never returns a fallback) on a bad scheme, at first access, so a
 * misconfigured production build can be obvious rather than discovered
 * live: THIS function alone only guarantees that much, though. Whether
 * "at first access" actually lands at boot, loud enough to be noticed,
 * depends on the callers (src/app/_layout.tsx's boot-time touch, and
 * src/api/client.ts hoisting its own read above its fetch try/catch), see
 * the module docstring above. Never includes the full configured value in
 * the thrown message (only the scheme): the value can carry embedded
 * credentials.
 */
function requireHttpsInProduction(url: string, isProd: boolean): string {
  if (!isProd) return url;
  // Item 6 (LOW, adversarial safety review, 2026-08-04): `schemeOf` trims
  // before checking, but this used to return the ORIGINAL, un-trimmed
  // `url` on a pass, so `"  https://host  "` / a leading tab / a leading
  // U+FEFF / a trailing newline all read as "fine" here and were handed to
  // `fetch` verbatim, where they always fail as a plain `network_error`
  // (a malformed URL never reaches the network). The gate said "fine"
  // about a value it had just proven needed cleaning. Trim once, check and
  // return the SAME trimmed value, so a pass from this function is an
  // actual guarantee about what gets used.
...
export const env = {
   ...
   *  (`requireHttpsInProduction` above, #292); whether that throw actually
   *  lands where it's noticed (boot, not a swallowed catch) is on the
   *  callers, see the module docstring above. */
  get apiUrl(): string {
