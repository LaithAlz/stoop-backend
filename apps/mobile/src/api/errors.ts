...
export function toHouseApiError(error: ApiError): string {
  switch (error.code) {
    ...
    // shipped client sends that shape today (this screen's own onboarding
    // backup step, src/app/onboarding/backup.tsx, already requires a name
    // AND a phone together, or sends neither), so this was unmapped and
    // fell through to the generic retry line below, telling a landlord to
