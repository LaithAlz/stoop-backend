...
| `test_downgrade_to_0004_removes_role_policies_and_grants` used to fail — `app_role` still exists after downgrade — on a multi-DB local cluster (fixed #193) | If you still see this on a CURRENT checkout: does `SELECT DISTINCT d.datname FROM pg_shdepend sd JOIN pg_database d ON d.oid = sd.dbid WHERE sd.refclassid = 'pg_authid'::regclass AND sd.refobjid = 'app_role'::regrole AND sd.dbid <> (SELECT oid FROM pg_database WHERE datname = current_database())` return any sibling database? | Postgres roles are CLUSTER-global. The 0005 downgrade's `pg_shdepend` guard sees a sibling database's grants still referencing `app_role` and — by design — leaves the role in place instead of failing the downgrade; the test's ORIGINAL single-database assumption then found "leftover" state and failed | **#193 fix, landed**: the test now runs that same detection query itself and `pytest.skip`s with an explanatory message when sibling-DB grants exist, instead of asserting the drop — CI (single DB, no siblings) still exercises the full drop assertion every time. If you see a FAILURE (not a skip) on a genuinely single-DB run, that is a real regression, not this old topology issue | `migrations/versions/0005_app_role_and_rls.py` (DOWNGRADE + `pg_shdepend` guard docstring); `tests/test_migrations_0005.py::test_downgrade_to_0004_removes_role_policies_and_grants` (`_other_databases_holding_app_role_grants` detection helper); `gh issue view 193 --repo LaithAlz/stoop-backend`; the original `stoop_review` incident (session-verified 2026-07-05; no repo artifact) |
...
| A `*_double_timeout_shares_one_end_to_end_deadline` test fails — used to be a wall-clock assertion (`assert elapsed < 0.6`) (fixed #212 item 2) | If you still see a real-elapsed-time assertion in the diff, that's a regression — the fix removed `time.monotonic()`-measured assertions from these three tests entirely | **Was a load-sensitive flake, not a deadline bug** (noted in the #187/#188 review rounds, 2026-07-12): these tests shrunk `CLASSIFICATION_BUDGET_SECONDS` to an artificial 0.3 s and asserted total elapsed < 0.6 s — a loaded machine (parallel agents, Docker churn, full-suite contention) could eat the margin | **#212 item 2 fix, landed**: `app/integrations/anthropic.py`'s `_now()` seam (already there for exactly this) is monkeypatched with a scripted `_ScriptedClock`, and `call_tool_forced` is wrapped to record each attempt's actual `timeout_seconds` and advance that clock by it — the assertion now pins the exact per-attempt timeout values (first capped, second gets the shared deadline's remainder) with ZERO dependence on real elapsed time, so this class of flake cannot recur here. If one of these three tests fails now, it is a genuine shared-deadline regression, not scheduling jitter | the deadline-sharing tests (three as of 2026-07-12, converted 2026-07-18) in `tests/test_agent_classify_intent.py`, `tests/test_agent_classify_severity.py`, `tests/test_agent_draft_response.py` (`_ScriptedClock` / `_record_call_tool_forced_timeouts` helpers in each); `gh issue view 212 --repo LaithAlz/stoop-backend` |
