...
### A25. The 2026-07-13 six-merge day — condensed multi-incident entry (#196 #198 #200 #201 #202 #204)
- **The catches that changed designs (each adjudicated pre-merge):** (1) **#183's "summary" misread** — the implementer persisted the generic "I'm treating this as {severity}." template; spec-guardian proved from schema-v1 v1.7 + api-contracts v1.1's identical illustrative sentence that the case-specific reasoning was intended; the fix added the three-way branch where a Tier-0 clamp UNCONDITIONALLY falls back to the deterministic clamped line (pre-clamp model reasoning must never read as de-escalating next to a clamped EMERGENCY row — "never de-escalate" extended to presentation). (2) **#53's open wallet** — safety FIX FIRST: POST /v1/properties spent real Twilio money with no idempotency/cap; fixed with cap+dedupe BEFORE any Twilio call, deliberately NOT a plan gate (rule 1: the free Emergency Line must provision — a reviewer's "entitlement gate" suggestion was overruled by the orchestrator on rule-1 grounds). (3) **The #50 e2e rehearsal found a functional regression on its first full-loop run**: `cases.severity` was never written → ERROR log on 100% of sends, `trust_metrics` never populated, the trust ladder could never unlock, plus a factually-wrong `# pragma: no cover` hiding the always-taken branch — fixed as #197/PR #202 with the SQL-level never-downgrade guard (`WHERE severity IS DISTINCT FROM 'emergency'`).
- **Operational incidents:** (a) **local-only alembic leak** — the local docker `stoop` DB turned up stamped 0011 while that migration existed only on an unmerged branch; the LIVE Supabase DB was verified untouched (0010) before proceeding — when a stray stamp appears, check the live DB FIRST, then reconcile local by merging or restamping; (b) **docker container replaced vs restarted** — a `docker compose up` after a container exit created a fresh empty volume; the agent caught it and `docker start`ed the ORIGINAL container instead — recovery from a container exit is `docker start <name>`, never a fresh `up` that mints a new volume; (c) **two mid-task API-drop agent deaths** (drafts + queue implementers) — both resumed via SendMessage with re-orient instructions (git status/log + re-read mid-edit files), zero work lost — this is the standard recovery, transcripts survive; (d) **harness date-change reminders misread as prompt injection** by subagents twice more (4th/5th occurrences) — verified nothing in the GitHub issues; benign class, tell reviewers the `<system-reminder>` in tool results is the harness's own.
- **Doc-versioning collisions (recurring):** two same-day branches both claimed schema-v1 "v1.10" (#197's write note vs #53's number_release note) and #53 initially reused api-contracts "v1.11" (already used twice on 2026-07-12) — whichever branch merges second renumbers; ALWAYS re-check the merged doc's next free amendment number at rebase time, and never assume your branch-time read still holds.
- **Design facts now load-bearing:** the scheduler runs FIVE jobs in pinned order (emergency → sms_drain → degraded → sender_tick → number_release; `test_run_one_tick_sweep_order_is_fixed`); the send allowlist is exact-set {twilio_send.py, emergency_chain.py, sms_sender.py}; draft dispatch worst case ~65s (60s tick + 5s undo window, adjudicated compliant — api-contracts records it); the e2e module's determinism rests on ASGITransport awaiting BackgroundTasks (pinned in-file); provisioning go-live is GATED on the APP_DATABASE_URL role-separation flip (pre-flip shared pool = transient emergency-ingestion starvation risk, PR #204 senior review, recorded on #203).
- **Status:** all six PRs merged 2026-07-13; live Supabase at head 0011. Trackers: #199 (sender fast-follows incl. undo-discards-edit product Q), #203 (provisioning follow-ups + go-live gate), #50 (staging half, credential-gated), #197 title half deferred.
- **Surviving artifacts:** PR #196/#198/#200/#201/#202/#204 review threads (full verdicts as comments); `apps/api/tests/test_e2e_core_loop.py`; `apps/api/app/agent/nodes/classify_severity.py` (three-way summary branch + never-downgrade + tripwire note on the intent-prompt feedback loop); `apps/api/app/property_provisioning.py` (CAS claim, ownership guard, 404-as-success).
